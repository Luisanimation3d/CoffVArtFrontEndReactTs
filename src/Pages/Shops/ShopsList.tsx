...
export const Shops = () => {
	...
	if (search.length > 0) {
		dataShopsFiltered = dataShopsModify.filter(
			(shops: any) =>
	...
	useEffect(() => {
		if (data?.shops?.rows) {
			const newData = data?.shops?.rows?.map((item: any) => ({
				...
				State: (
					<Button
						text={item.state ? 'Activo' : 'Anulada'}
						autosize={false}
						type={'BUTTON'}
						onClick={(e: any) => {
							e.stopPropagation();
						...
						<Modal showModal={setIsModalOpen}>
							<div className='invoice-container'>
								...
								<div className='client-info'>
									...
									<p>Proveedor: {shopsDetails[0]?.supplier}</p>
									<p>Fecha de compra: {shopsDetails[0]?.date}</p>
									<p>Descripción: {shopsDetails[0]?.description}</p>
								</div>
