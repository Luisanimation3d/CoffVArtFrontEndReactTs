...
export const CreateSupply = () => {
    ...
    const handleSubmit= async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        let mensajeError = {}
        if (!formData.name || formData.name.trim().length < 4 || !/^[a-zA-Z\d\u00C0-\u017F\s]+$/ .test(formData.name)) {
            mensajeError = {...mensajeError, name: 'El nombre es requerido'}
        }
        if (!formData.description || formData.description.trim().length < 5 || !/^[a-zA-Z\d\u00C0-\u017F\s]+$/ .test(formData.description)){
