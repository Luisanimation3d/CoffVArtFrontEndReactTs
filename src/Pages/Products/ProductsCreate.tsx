...
export const CreateProduct = () => {
	...
	const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
		e.preventDefault();
		let mensajeError = {};
		if (!formData.name || formData.name.trim().length < 1 || !/^[a-zA-ZÁÉÍÓÚáéíóú\s0-9]+$/.test(formData.name)) {
		...
		if (!formData.description || formData.description.trim().length < 1 || !/^[a-zA-ZÁÉÍÓÚáéíóú\s()0-9]+$/.test(formData.description)) {
			mensajeError = {
				...mensajeError,
