...
export const Orders = () => {
	...
	if (search.length > 0) {
		dataOrdersFiltered = dataOrdersModify.filter(
			(order: any) =>
				order.code.toLowerCase().includes(search) ||
				order.state.toLowerCase().includes(search.toLowerCase()) ||
				order.coustumer.name.toLowerCase().includes(search.toLowerCase())
		);
	} else {
						...
						<Modal showModal={setIsModalOpen}>
							<div className='invoice-container'>
								...
								<div className='client-info'>
									...
									<br />
									<p>
										Documento: {orderDetails[0]?.documentType}{' '}
										{orderDetails[0]?.document}
									</p>
								</div>
